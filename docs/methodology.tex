...
This dissertation employs a controlled audit design. A fixed battery of mental health scenarios is presented to multiple large language models, with only the stated city and country of the user varied across queries. All other elements of the prompt --- language, scenario content, tone, and phrasing --- are held constant. This design isolates the effect of stated geographic location from variation in query content, enabling direct attribution of response differences to location. The study combines automated content coding across all 1,120 responses with structured human validation on a stratified subsample, following a pre-registered analysis plan. The key methodological commitment is that no automated outcome measure is used in a hypothesis test until it has been independently validated against human labels, with a pre-specified F1 threshold of 0.70.
