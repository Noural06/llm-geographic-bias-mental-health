...
\textbf{Risk.} The crisis-contact audit (Section 4.7) identifies and reproduces examples of numbers with visibly suspicious patterns. There is a risk that reproducing them could propagate a non-functional contact if an excerpt is separated from its warning label.

\textbf{Mitigation.} Examples in Appendix D.4 are labelled as visibly suspicious pattern matches and are never presented as genuine guidance. The audit distinguishes verified, general-emergency, visibly suspicious and unresolved contacts; it does not equate an unmatched number with fabrication.
