...
The following exploratory analyses characterise the response corpus prior to any hypothesis testing. No confirmatory inferences are drawn here.
...
\begin{table}[H]
\centering
\caption{Descriptive statistics by income category ($n = 280$ per group). Specificity means phone numbers, short codes, and URLs per response; values corrected from the pipeline rerun.}
...
\textbf{Interpretation:} High-income cities receive responses scoring 0.614 points higher on the 0--5 actionability scale than low-income cities, after controlling for model identity and scenario. The effect is not attributable to response length (equal across tiers) or to model selection (it survives all seven model-exclusion variants).

\textbf{Limitation:} Actionability is measured as the presence of concrete components, not their accuracy. A response with a visibly suspicious crisis number scores equally to one with a verified real number.
...
\textbf{Data:} Same 1,120 responses; \texttt{comp\_contact\_verified} constructed from the verification table. Of 490 responses containing a detected contact, 68 (13.9\%) matched a documented service; all 68 occurred in the established-national-line tier.
